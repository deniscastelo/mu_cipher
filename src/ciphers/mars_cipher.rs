...
impl MarsCipher {
    ...
    pub fn new(key: &[u8]) -> Self {
        let mut cipher = MarsCipher { l_key: [0u32; 40] };
    ...
    fn set_key(&mut self, in_key: &[u8]) {
        ...
        let key_len = in_key.len() * 8;

        for i in 0..in_key.len() / 4 {
            t[i] = self.to_u32_little_endian(&in_key[0..4]);
        }

        t[in_key.len() / 4] = in_key.len() as u32 / 4;

        for j in 0..4 {
            for i in 0..15 {
                t[i] =
                    self.rot_l(t[(i + 8) % 15] ^ t[(i + 13) % 15], 3) ^ (4 * i as u32 + j as u32);
            }

            for _ in 0..4 {
                for i in 0..15 {
                    let index = (t[(i + 14) % 15] as usize) % 512;
                    t[i] = self.rot_l(t[i].wrapping_add(self::S_BOX[index]), 9);
                }
            }

            for i in 0..10 {
                self.l_key[10 * j + i] = t[(4 * i) % 15];
            }
        }

        for i in (5..37).step_by(2) {
            let mut w = self.l_key[i] | 3;
            let m = self.gen_mask(w);

            if m != 0 {
                w ^= self.rot_l(
                    self::S_BOX[265 + (self.l_key[i] & 3) as usize],
                    (self.l_key[i - 1] & 31) as u32,
                ) & m;
            }
    ...
    fn to_u32_little_endian(&self, buffer: &[u8]) -> u32 {
        u32::from_le_bytes(buffer.try_into().unwrap())
    }

    fn write_uint32_little_endian(&self, value: u32, buffer: &mut [u8], offset: usize) {
    ...
    fn rot_l(&self, x: u32, y: u32) -> u32 {
        (x << y) | (x >> (32 - y))
    }

    fn rot_r(&self, x: u32, y: u32) -> u32 {
        (x >> y) | (x << (32 - y))
    }

    fn gen_mask(&self, x: u32) -> u32 {
        let mut m = (!x ^ (x >> 1)) & 0x7fffffff;
    ...
    fn f_mix(&self, a: &mut u32, b: &mut u32, c: &mut u32, d: &mut u32) {
        let mut r = self.rot_r(*a, 8);
        *b ^= self::S_BOX[(*a & 255) as usize];
        *b += self::S_BOX[((r & 255) + 256) as usize];
        r = self.rot_r(*a, 16);
        *a = self.rot_r(*a, 24);
        *c += self::S_BOX[(r & 255) as usize];
        *d ^= self::S_BOX[((*a & 255) + 256) as usize];
    }

    fn b_mix(&self, a: &mut u32, b: &mut u32, c: &mut u32, d: &mut u32) {
        let mut r = self.rot_l(*a, 8);
        *b ^= self::S_BOX[((*a & 255) + 256) as usize];
        *c -= self::S_BOX[(r & 255) as usize];
        r = self.rot_l(*a, 16);
        *a = self.rot_l(*a, 24);
        *d -= self::S_BOX[((r & 255) + 256) as usize];
        *d ^= self::S_BOX[(*a & 255) as usize];
    }

    fn f_ktr(&self, a: &mut u32, b: &mut u32, c: &mut u32, d: &mut u32, i: usize) {
        let m = *a + self.l_key[i];
        *a = self.rot_l(*a, 13);
        let mut r = *a * self.l_key[i + 1];
        let mut l = self::S_BOX[(m & 511) as usize];
        r = self.rot_l(r, 5);
        *c += self.rot_l(m, r & 31);
        l ^= r;
        r = self.rot_l(r, 5);
    ...
    fn r_ktr(&self, a: &mut u32, b: &mut u32, c: &mut u32, d: &mut u32, i: usize) {
        let mut r = *a * self.l_key[i + 1];
        *a = self.rot_r(*a, 13);
        let m = *a + self.l_key[i];
        let mut l = self::S_BOX[(m & 511) as usize];
    ...
    fn decrypt_block(&self, in_buf: &[u8], out_buf: &mut [u8]) {
        let mut d = self.to_u32_little_endian(&in_buf[0..4]) + self.l_key[36];
        let mut c = self.to_u32_little_endian(&in_buf[4..8]) + self.l_key[37];
        let mut b = self.to_u32_little_endian(&in_buf[8..12]) + self.l_key[38];
        let mut a = self.to_u32_little_endian(&in_buf[12..16]) + self.l_key[39];

        self.f_mix(&mut a, &mut b, &mut c, &mut d);
        a += d;
        self.f_mix(&mut b, &mut c, &mut d, &mut a);
        b += c;
        self.f_mix(&mut c, &mut d, &mut a, &mut b);
        self.f_mix(&mut d, &mut a, &mut b, &mut c);
        self.f_mix(&mut a, &mut b, &mut c, &mut d);
        a += d;
        self.f_mix(&mut b, &mut c, &mut d, &mut a);
        b += c;
        ...
        self.r_ktr(&mut a, &mut b, &mut c, &mut d, 34);
        self.r_ktr(&mut b, &mut c, &mut d, &mut a, 32);
        self.r_ktr(&mut c, &mut d, &mut a, &mut b, 30);
        self.r_ktr(&mut d, &mut a, &mut b, &mut c, 28);
        self.r_ktr(&mut a, &mut b, &mut c, &mut d, 26);
        self.r_ktr(&mut b, &mut c, &mut d, &mut a, 24);
        self.r_ktr(&mut c, &mut d, &mut a, &mut b, 22);
        self.r_ktr(&mut d, &mut a, &mut b, &mut c, 20);
        self.r_ktr(&mut a, &mut b, &mut c, &mut d, 18);
        self.r_ktr(&mut b, &mut c, &mut d, &mut a, 16);
        self.r_ktr(&mut c, &mut d, &mut a, &mut b, 14);
        self.r_ktr(&mut d, &mut a, &mut b, &mut c, 12);
        self.r_ktr(&mut a, &mut d, &mut c, &mut b, 10);
        self.r_ktr(&mut b, &mut a, &mut d, &mut c, 8);
        self.r_ktr(&mut c, &mut b, &mut a, &mut d, 6);
        self.r_ktr(&mut d, &mut a, &mut b, &mut c, 4);

        self.b_mix(&mut a, &mut b, &mut c, &mut d);
        self.b_mix(&mut b, &mut c, &mut d, &mut a);
        c -= b;
        self.b_mix(&mut c, &mut d, &mut a, &mut b);
        d -= a;
        self.b_mix(&mut d, &mut a, &mut b, &mut c);
        self.b_mix(&mut a, &mut b, &mut c, &mut d);
        self.b_mix(&mut b, &mut c, &mut d, &mut a);
        c -= b;
        self.b_mix(&mut c, &mut d, &mut a, &mut b);
        ...
        out_words[0] = d - self.l_key[0];
        out_words[1] = c - self.l_key[1];
        out_words[2] = b - self.l_key[2];
        out_words[3] = a - self.l_key[3];

        self.write_uint32_little_endian(out_words[0], out_buf, 0);
        self.write_uint32_little_endian(out_words[1], out_buf, 4);
        self.write_uint32_little_endian(out_words[2], out_buf, 8);
        self.write_uint32_little_endian(out_words[3], out_buf, 12);
    }
}

impl ICipher for MarsCipher {
